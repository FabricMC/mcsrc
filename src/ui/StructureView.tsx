  ...
  const { treeData, tokenByKey } = useMemo(() => {
    ...
    const nodes: StructureNode[] = classes.map((className) => {
      ...
      const classNode: StructureNode = {
        key: classKey,
        title: (
          <span className="structure-title" title={className}>
            {classDisplayName}
          </span>
        ),
        children: [],
      ...
      classNode.children = methods.map((method) => {
        ...
        return {
          key: methodKey,
          title: (
            <span className="structure-title" title={`${method.name}${method.descriptor}`}>
              {formatMethodDisplayName(method, classDisplayName)}
            </span>
          ),
          isLeaf: true,
          token: method,
