...
const Code = () => {
  ...
  function applyTokenDecorations(model: editor.ITextModel) {
    ...
    if (editorRef.current && decompileResult.tokens) {
      const decorations = decompileResult.tokens.map((token) => {
        ...
        const canGoTo =
          !token.declaration && classList && classList.includes(token.className + ".class");
  ...
  useEffect(() => {
    const sub = selectedFile
      .pipe(startWith(selectedFile.value), pairwise())
      .subscribe(([prev, curr]) => {
        if (prev === curr) return;
      ...
      <Editor
        ...
        onMount={(codeEditor) => {
          ...
          codeEditor.onDidChangeCursorPosition((e) => {
            ...
            const validToken =
              token != null &&
              (token.type == "class" || token.type == "method" || token.type == "field");
