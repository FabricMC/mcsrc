...
const getIndexer = async (): Promise<Indexer> => {
  if (!indexerFunc) {
    try {
      ...
      console.warn(
        "Failed to load WASM module (non-compliant browser?), falling back to JS implementation",
        e,
      );
      indexerFunc =
        (await import("../../java/build/generated/teavm/js/java.js")) as unknown as Indexer;
    }
  }
  return indexerFunc;
};
