...
function createWrorker() {
  return new ComlinkWorker<DecompileWorker>(new URL("./worker", import.meta.url), {
...
async function ensureWorkers(count: number) {
  ...
  let newWorkers = Array.from({ length: count - workers.length }, () => createWrorker());
...
export function decompileEntireJar(
  jar: Jar,
  options?: DecompileEntireJarOptions,
): DecompileEntireJarTask {
  const sab = new SharedArrayBuffer(Uint32Array.BYTES_PER_ELEMENT);
  const state = new Uint32Array(sab);
  state[0] = 0;
  ...
  return {
    async start() {
      try {
        ...
        let current = 0;
        const optLogger = options?.logger
          ? Comlink.proxy((i: number) => {
