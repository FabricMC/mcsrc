import { editor, languages, type CancellationToken, type IDisposable } from "monaco-editor";
...
function formatMarkdownAsHtml(md: string, token: Token): string {
  ...
  const nestingLevel =
    (token.className.match(/\$/g) || []).length +
    (token.type == "method" || token.type == "field" ? 1 : 0);
