...
export const calculatedLineChanges = new BehaviorSubject<
...
export function countLineDiff(
  ...
  if (
    oldText.startsWith("// Class not found") ||
    oldText.startsWith("// Error during decompilation")
  ) {
    const newLines = newText === "" ? 0 : newText.split(/\r?\n/).length;
    return { additions: newLines, deletions: 0 };
  }
  if (
    newText.startsWith("// Class not found") ||
    newText.startsWith("// Error during decompilation")
  ) {
