...
setTimeout(() => {
  combineLatest([getLeftDiff().selectedVersion, selectedMinecraftVersion]).subscribe(() => {
  ...
  for (const key of allKeys) {
    ...
    if (leftInfo === undefined) {
      ...
      const hasChanges =
        ...
        Array.from(leftClasses.entries()).some(
          ([className, leftCrc]) => rightClasses.get(className) !== leftCrc,
        );
