...
const decompilerOptions = combineLatest([displayLambdas.observable]).pipe(
  distinctUntilChanged(),
...
export function decompileResultPipeline(
  jar: Observable<MinecraftJar>,
): Observable<DecompileResult> {
  return combineLatest([selectedFile, jar, bytecode.observable, decompilerOptions]).pipe(
    distinctUntilChanged(),
    throttleTime(250),
